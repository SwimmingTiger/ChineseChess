...
* @brief 向上移动光标的键值表
* 
* 一个二维键值数组（struct KeyCode key[KEYCODE_MAX_NUM]）。
* struct KeyCode的第一个成员为key[x].first，第二个成员为key[x].second。
...
struct KeyCode
{
    signed char first; ///< 第一字节（键值单字节这里存键值，键值双字节这里存后一个字节）
    signed char second; ///< 第二字节（键值单字节这里为-1，键值双字节这里存前一个字节）
};
